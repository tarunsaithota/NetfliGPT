...
const MovieInfo = () => {
  ...
  return (
    <div ref={topOfDetails} tabIndex="-1">
      <div className='md:hidden fixed -z-10 opacity-50'>
        <img src={POSTER_CDN+movieInfo?.backdrop_path} alt='backdropImage' className='w-screen object-cover'/>
        </div>
      ...
      <div className="w-full p-2 py-8 md:pt-16">
          <div className='px-8 pt-2 md:pt-6 md:flex md:w-full'>
            <div className='w-28 md:w-[25%] mr-8 shadow-lg mb-4 md:mb-0 pt-16 md:pt-0'>
              <img src={POSTER_CDN+movieInfo?.poster_path } alt='Movie Card' className='rounded-xl w-full h-full object-cover'/>
            </div>
            <div className='md:px-8 pt-3 text-blue-950 md:w-[70%] md:ml-8 md:shadow-inner'>
            <p className="text-lg md:text-4xl font-extrabold py-2 md:py-5  md:text-blue-950">
            {movieInfo?.title}🍿
          </p>
              ...
              <p className='pt-2 md:pt-6 pb-4 md:pb-10 shadow-xs font-semibold'>{movieInfo?.overview}</p>
              <button className=' bg-blue-950 hover:bg-blue-800 w-32 md:w-36 h-10 text-white rounded-lg' onClick={handlePlayButton}>▶️ Play Trailer</button>
            </div>
          </div>
      </div>
      {displayTrailer && (
